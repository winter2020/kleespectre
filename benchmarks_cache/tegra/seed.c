 ...
 */
u8 byte(const u32 x, const unsigned n)
{
	return x >> (n << 3);
}
...
int seed_set_key(struct seed_ctx *ctx, const u8 *in_key,
...
static void seed_encrypt(struct seed_ctx *ctx, u8 *out, const u8 *in)
{
...
static void seed_decrypt(struct seed_ctx *ctx, u8 *out, const u8 *in)
{
	const __be32 *src = (const __be32 *)in;
	__be32 *dst = (__be32 *)out;
	u32 x1, x2, x3, x4, t0, t1;

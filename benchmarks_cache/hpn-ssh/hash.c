...
unsigned char victim_fun2(int idx) __attribute__ ((optnone)) {
    unsigned char temp = 0;
    if (idx < array1_size) {    
        temp &= array2[array1[idx]];
    }   
...
unsigned char victim_fun3(int idx) __attribute__ ((optnone)) {
    unsigned char  temp = 0;
    if (idx < array1_size) {    
        temp &= array2[array1[idx]];
    }   
...
void store_bigendian(unsigned char *x,uint64 u) 
{
  x[7] = u; u >>= 8;
  x[6] = u; u >>= 8;
  x[5] = u; u >>= 8;
...
typedef unsigned long long uint64;
unsigned char buff[497*64]__attribute__((aligned(64)));;

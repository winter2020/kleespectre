...
static uint32_t get_unaligned_be32(const void *_buf)
{
...
void camellia_setup_tail(u32 *subkey, u32 *subL, u32 *subR, int max)
{
	u32 dw, tl, tr;
	u32 kw4l, kw4r;
...
void camellia_do_decrypt(const u32 *subkey, u32 *io, unsigned i)
{
	u32 il, ir, t0, t1;            /* temporary variables */
...
struct camellia_ctx came_ctx;

int camellia_set_key(const u8 *in_key, unsigned int key_len)
{
	struct camellia_ctx *cctx = &came_ctx;
...
// static void camellia_encrypt(struct camellia_ctx *cctx, u8 *out, const u8 *in)
// {
...
// static void camellia_decrypt(struct camellia_ctx *cctx, u8 *out, const u8 *in)
// {
// 	const __be32 *src = (const __be32 *)in;
// 	__be32 *dst = (__be32 *)out;
// 	unsigned int max;
...
int main() {
    ...
    //camellia_set_key(key, 24);
    //camellia_set_key(key, 32);
    //camellia_set_key(key, 0);

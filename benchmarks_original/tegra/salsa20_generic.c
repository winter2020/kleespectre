...
void crypto_xor_byte(u8 *a, const u8 *b, unsigned int size)
{
	for (; size; size--)
		*a++ ^= *b++;
}
...
void salsa20_keysetup(struct salsa20_ctx *ctx, const u8 *k, u32 kbytes)
{
...
// static int setkey(struct salsa20_ctx *ctx, const u8 *key,

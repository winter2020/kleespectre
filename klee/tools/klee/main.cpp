...
int main(int argc, char **argv, char **envp) {
  ...
  { // output end and elapsed time
    ...
    uint64_t seconds = time::seconds(endTime - startTime).toMicroseconds();
  ...
  uint64_t loads = 
      *theStatisticManager->getStatisticByName("Loads");

  uint64_t stores = 
      *theStatisticManager->getStatisticByName("Stores");

  uint64_t constLoads = 
      *theStatisticManager->getStatisticByName("Cloads");

  uint64_t constStores = 
      *theStatisticManager->getStatisticByName("Cstores");
  uint64_t spStates = 
      *theStatisticManager->getStatisticByName("Spstates");

  uint64_t spStatesDone = 
      *theStatisticManager->getStatisticByName("CompleteSpstates");

  uint64_t totalSpecInst = 
      *theStatisticManager->getStatisticByName("TotalSpecInst");
  ...
  stats << "KLEE: done: sp states = " 
        <<  spStates << "\n";

  stats << "KLEE: done: Completed sp states = " 
        <<  spStatesDone << "\n";

    if (spStatesDone > 0) {
        stats << "KLEE: done: Average instructions on speculative path = " 
              <<  totalSpecInst * 1.0 / spStatesDone << "\n";
    }
 
  stats << "KLEE: done: generated tests = "
        << handler->getNumTestCases() << "\n";

  stats <<"KLEE: done: loads: "
        << loads << "\n";
  stats <<"KLEE: done: stores: "
        << stores << "\n";
  stats <<"KLEE: done: constant loads: "
        << constLoads << "\n";
  stats <<"KLEE: done: constant stores: "
        << constStores << "\n";

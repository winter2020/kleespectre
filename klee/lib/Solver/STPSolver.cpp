        ...
        runAndGetCexForked(::VC vc, STPBuilder *builder, ::VCExpr q,
            ...
            if (pid == 0) {
                if (timeout) {
                    ...
                    unsigned t = std::max(1u, static_cast<unsigned>(timeout.toSeconds()));
                    ::alarm(t);
                }
                ...
                //klee_warning("Child process exit normally!");
                _exit(res);
                ...
                //////////////////
                if (false && timeout) {
                    int tm = 1 * (timeout.toSeconds()) + 1;
                    int kills = 3;
                    int ret;

                    //klee_message("Time out: %d", tm);
                    ...
                    struct timespec rem;
                    // check every 200 ms
                    //llvm::errs() << "T: " << std::time(0) << "\n";
                    do {
                        res = waitpid(pid, &status, WNOHANG);
                        if (res == 0) {
                            if (tm > 0) {
                                //llvm::errs() << "Time s: " << std::time(0) << "\n";
                                do {
                                    //klee_message("time: %ld", req.tv_nsec);
                                    //ret = nanosleep(&req, &rem);
                                    //req.tv_sec = rem.tv_sec;
                                    //req.tv_nsec = rem.tv_nsec;
                                } while(ret == -1);
                                //llvm::errs() << "Time e: " << std::time(0) << "\n";
                                tm--;
                            } else {
                                if (kills > 0) {
                                    llvm::errs() << "Time e1: " << std::time(0) << "\n";
                                    kill(pid, SIGKILL);
                                    klee_warning("Kill solver %d", 3 - kills);
                                    req.tv_sec = 1;
                                    req.tv_nsec = 0;
                                    do {
                                        ret = nanosleep(&req, &rem);
                    ...
                    //int diff = std::time(0) - start;
                    //llvm::errs() << "Time e: " << diff << "\n";
                }

                //klee_message("Status: %d", status);
                if (res < 0) {
                ...
                //klee_message("exitcode=%d", exitcode);

...
/*
static void getAllReachableFunctions(KModule *km) {
    std::set<KFunction *> all_reachable_functions;
    std::queue<KFunction *> reached_functions;

    for (auto &kfp: km->functions) {
        KFunction *kf = kfp.get();
        if (kf->function->getName() == "__klee_posix_wrapped_main") {
            llvm::errs() << "Find posix_main \n";
            all_reachable_functions.clear();
            while(!reached_functions.empty()) {reached_functions.pop();}
            all_reachable_functions.insert(kf);
            reached_functions.push(kf);
            break;
    ...
    while(!reached_functions.empty()) {
        KFunction *kf = reached_functions.front();
        llvm::errs() << "@@@ " << kf->function->getName() << "\n";
        for (unsigned i= 0; i < kf->numInstructions; ++i) {
            ...
            if (inst->getOpcode() == Instruction::Call) {
                ...
                Constant *c = dyn_cast<Constant>(fp);
                if (!c) {
                    continue;
                }
                ...
                if (km->functionMap.find(f) != km->functionMap.end()) {
                    KFunction *newkf = km->functionMap[f];
                    if (all_reachable_functions.find(newkf) == all_reachable_functions.end()) {
                        llvm::errs() <<"Add new function: " << f->getName() << "\n";
                        reached_functions.push(newkf);
                        all_reachable_functions.insert(newkf);
                    }
                } 
            }
        }
        reached_functions.pop();
    }

    for (auto &kf : all_reachable_functions) {
        llvm::errs() << "Reachable function: "  << kf->function->getName() << "\n";
  ...
  for (auto &kfp : km->functions) {
    ...
    for (unsigned i=0; i<kf->numInstructions; ++i) {
      ...
      if (OutputIStats) {
        ...
        if (kf->trackCoverage && instructionIsCoverable(ki->inst) && ki->info->isInUserCode) {
            ++stats::uncoveredInstructions;
        }
      }
      
      if (kf->trackCoverage && ki->info->isInUserCode) {
...
void StatsTracker::stepInstruction(ExecutionState &es) {
  if (OutputIStats) {
    ...
    if (sf.kf->trackCoverage && instructionIsCoverable(inst) && ii.isInUserCode) {

...
ExecutionState &DFSSearcher::selectState() {
    ExecutionState &ps = *states.back();
    if (ps.hasSpeStates()) {
        return ps.selectSpState();
    }
...
// Random Speculative Path Searcher
RandomPathSPSearcher::RandomPathSPSearcher(Executor &_executor)
  : executor(_executor) {
}

RandomPathSPSearcher::~RandomPathSPSearcher() {
}

ExecutionState &RandomPathSPSearcher::selectState() {
  unsigned flips=0, bits=0;
  PTree::Node *n = executor.processTree->root;
  while (!n->data) {
    if (!n->left) {
...
void
RandomPathSPSearcher::update(ExecutionState *current,
                           const std::vector<ExecutionState *> &addedStates,
                           const std::vector<ExecutionState *> &removedStates) {
}

bool RandomPathSPSearcher::empty() { 
  return executor.states.empty(); 
}

...
public:
  Expr() : isSensitive(false), refCount(0) { Expr::count++; }
...
protected:
  BinaryExpr(const ref<Expr> &l, const ref<Expr> &r) : left(l), right(r) {isSensitive = l.get()->isSensitive || r.get()->isSensitive;}
...
private:
  NotOptimizedExpr(const ref<Expr> &_src) : src(_src) {isSensitive=_src.get()->isSensitive;}
...
private:
  ConcatExpr(const ref<Expr> &l, const ref<Expr> &r) : left(l), right(r) {
    ...
    isSensitive = l.get()->isSensitive || r.get()->isSensitive;
  }
...
private:
  ExtractExpr(const ref<Expr> &e, unsigned b, Width w) 
    : expr(e),offset(b),width(w) {isSensitive=e.get()->isSensitive;}
...
private:
  NotExpr(const ref<Expr> &e) : expr(e) {isSensitive=e.get()->isSensitive;}
...
public:
  CastExpr(const ref<Expr> &e, Width w) : src(e), width(w) {isSensitive=e.get()->isSensitive;}
